...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {
	    ...
	    for (PointFeature eq: earthquakes) {
			...
			if (mag > highestMag || highestMag == -1.0f) {
				highestMag = mag;
			}
			if (mag < lowestMag || lowestMag == -1.0f) {
				lowestMag = mag;
			}
	    }
	    ...
	    for (PointFeature eq: earthquakes) {
	    	SimplePointMarker currentMarker = createMarker(eq);
	...
	*/
	private SimplePointMarker createMarker(PointFeature feature)
	{
	    ...
	    } else if (mag > THRESHOLD_MODERATE && mag < THRESHOLD_STRONG){
	    	marker.setColor(magenta);
	    } else {
	    	marker.setColor(red);
	    }

	    float markerRadius = map(mag, lowestMag, highestMag, 10.0f, 25.0f);

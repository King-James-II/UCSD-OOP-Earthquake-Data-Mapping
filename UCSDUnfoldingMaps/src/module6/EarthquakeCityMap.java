...
public class EarthquakeCityMap extends PApplet {
	...
	private void sortAndPrint(int numToPrint) {
		...
		if (numToPrint > quakearr.length) {
			for (EarthquakeMarker eq: quakearr) {
				System.out.println(eq);
			}
		}
		else if (numToPrint <= quakeMarkers.size()) {
			for (int i=0; i<= numToPrint-1; i++) {
				System.out.println(quakearr[i]);
			}
		}
	}
	...
	private void addFilterButtons() {
		...
		textAlign(LEFT, CENTER);
		textSize(12);
		text("Filters", xbase+25, ybase+25);
		...
		rect(45, 392, tri_xbase + 50, 15, 360);


		fill(0, 0, 0);
		text("Land Quake", xbase+50, ybase+70);
		fill(128, 128, 128, 127);
		rect(45, 412, tri_xbase + 50, 15, 360);
		fill(0, 0, 0);
		text("Ocean Quake", xbase+50, ybase+90);
		fill(128, 128, 128, 127);
		rect(45, 432, tri_xbase + 50, 15, 360);
	...
	public void mousePressed() {
		...
		if (filterButtonClicked == true) {
			...
			if (isPointInRect(mouseX, mouseY, 45, 392, 105, 15)) {
			...
			if (isPointInRect(mouseX, mouseY, 45, 412, 105, 15)) {
			    filterButtonClicked = true;
				for (Marker m : quakeMarkers) {
			...
			if (isPointInRect(mouseX, mouseY, 45, 432, 105, 15)) {
				filterButtonClicked = true;
				for (Marker m : quakeMarkers) {
		...
		boolean isPointInRect(float px, float py, float rx, float ry, float rw, float rh) {
		  // Check if point is in the bounding rectangle

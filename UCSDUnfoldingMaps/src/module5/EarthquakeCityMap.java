...
public class EarthquakeCityMap extends PApplet {
	...
	private void selectMarkerIfHover(List<Marker> markers)
	{
		...
		for (Marker m : markers) {
			...
			if (!cm.isHidden() && cm.isInside(map, mouseX, mouseY) && lastSelected == null) {
				cm.setSelected(true);
				lastSelected = cm;
			}
			else {
	...
	{
	    if (lastClicked != null) {
	        // Unhide all markers if the same marker is clicked again
	        lastClicked.setClicked(false);
	        lastClicked = null;
	        unhideMarkers();
	        if (lastClicked == lastSelected) {
	            return;
	        }
	    }
	    clickMarkerIfHover();
	...
	private void clickMarkerIfHover() {
			...
			if (lastSelected instanceof EarthquakeMarker) {
				...
				if (em.getClicked()) {
					...
					for (Marker city : cityMarkers) {
						double threatKm = em.threatCircle();
						double distanceToCity = em.getDistanceTo(city.getLocation());
						if (distanceToCity > threatKm) {
							city.setHidden(true);
						}
						else {
				...
				if (cm.getClicked()) {
	                ...
	                for (Marker quake : quakeMarkers) {
	                    ...
	                    double threatKm = em.threatCircle();
	                    double distanceToCity = em.getDistanceTo(cm.getLocation());
	                    quake.setHidden(distanceToCity > threatKm);
	                }
				}
			}
		}
